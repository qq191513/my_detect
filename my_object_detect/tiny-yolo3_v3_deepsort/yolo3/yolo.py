...
class YOLO(object):
    ...
    def get_detect_boxes(self, image):
        box_list = []
        start = timer()
        ...
        detect_time_used =end - start
        print('detect_time_used:{:.3f}s'.format(detect_time_used))
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            i = 0
            out_boxes = [left + i, top + i, right - i, bottom - i]
            box_list.append(out_boxes)

        box_array = np.array(box_list)
        return  box_array, out_scores, out_classes
